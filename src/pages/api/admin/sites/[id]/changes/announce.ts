...
export const POST: APIRoute = async ({ params, request, redirect, locals }) => {
	...
	if (!parsed.ok) {
		return redirect(`/admin/units/${siteId}/layout/sidebar?change_error=${parsed.error}`);
	}

	const result = await announceRecentChangeOnGitHub(supabase, siteId, parsed.entry);
	if (!result.ok) {
		return redirect(`/admin/units/${siteId}/layout/sidebar?change_error=${result.error}`);
	}

	return redirect(`/admin/units/${siteId}/layout/sidebar?change_saved=1`);
};
