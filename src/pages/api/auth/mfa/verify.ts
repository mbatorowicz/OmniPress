...
export const POST: APIRoute = async ({ request, cookies, redirect }) => {
	...
	if (!code || !factorId) {
		return redirect('/auth/mfa?error=' + encodeURIComponent(auth.mfa.missingCode));
	}
	...
	if (challengeError || !challenge?.id) {
		return redirect('/auth/mfa?error=' + encodeURIComponent(auth.mfa.verifyFailed));
	}
	...
	if (error) {
		return redirect(
			'/auth/mfa?error=' + encodeURIComponent(mapAuthError(error.message) || auth.mfa.verifyFailed),
		);
	}
