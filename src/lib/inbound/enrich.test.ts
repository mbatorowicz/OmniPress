...
describe('enrichInboundDraft', () => {
	...
	it('jeden szkic ze wszystkimi plikami dwóch spraw → druga tura rozdziela', async () => {
		vi.spyOn(console, 'info').mockImplementation(() => {});
		const lumped = {
			intent: 'create',
			posts: [
				{
					title: 'Zaszczep pupila',
					category_slug: 'aktualnosci',
					content_md: 'Obowiązek szczepienia i obszar zagrożony.',
					attachments: [
		...
		const split = {
			...
			posts: [
				...
				{
					title: 'Wścieklizna - obszar zagrożony i zasady zachowania',
					category_slug: 'aktualnosci',
					content_md: 'Plakaty o obszarze zagrożonym i zasadach zachowania.',
					attachments: [
						{ filename: 'Plakat_wścieklizna_obszar_zagrożony.pdf', display: 'embed' },
						{ filename: 'Plakat_wścieklizna_zasady_zachowania.pdf', display: 'embed' },
					],
		...
		const complete = vi.fn().mockResolvedValueOnce(lumped).mockResolvedValueOnce(split);
		const attachments = [
			{
				filename: 'plakat_Zaszczep_pupila.jpg',
				mime: 'image/jpeg',
		...
		expect(complete.mock.calls[1]?.[0]?.prompt).toContain(inboundAi.splitRetry);
		expect(outcome.kind).toBe('create');
		if (outcome.kind === 'create') {
			expect(outcome.drafts).toHaveLength(2);
			expect(outcome.drafts[0]?.attachments).toHaveLength(1);
			expect(outcome.drafts[1]?.attachments).toHaveLength(2);
		}
		vi.restoreAllMocks();
	});
