...
13. **Webhook inbound** — `POST /api/inbound/email`. Brak sesji panelu: podpis Svix (`svix-id` / `svix-timestamp` / `svix-signature`) i `RESEND_WEBHOOK_SECRET`. Obcy From: 200 `{ ignored: true }`. Po ważnym podpisie i allowliście: 200 od razu, ingest w `waitUntil` (Hobby, `maxDuration` 60 s). Skutek: szkic `draft` (1–3), podmiana w panelu bez publikacji, albo pytanie mailem **tylko** na adres z allowlisty (envelope From). Jednostka z hopu przekazującego, nie z envelope. Nie logować treści maila (wolno: model, intent, liczba szkiców, kod błędu). Outbound do Vercel AI Gateway (Grok) z plikami; timeout / błąd = pytanie / Telegram, **nie** import 1:1 z tematu. Ponowny odczyt: `POST /api/admin/inbound/replay` (sesja administratora). Mail zwrotny: From `wpisy@inbound.cncsolutions.dev`, DKIM/SPF domeny inbound.
