...
| `src/lib/inbound/` | Webhook poczty — Svix, allowlista, szkic `draft` |
...
13. **Webhook inbound** — `POST /api/inbound/email`. Brak sesji panelu: podpis Svix (`svix-id` / `svix-timestamp` / `svix-signature`) i `RESEND_WEBHOOK_SECRET`. Obcy From: 200 `{ ignored: true }`. Po ważnym podpisie i allowliście: 200 od razu, ingest w `waitUntil` (Fluid Hobby, `maxDuration` 300 s). Skutek: jeden szkic `draft` na mail, podmiana w panelu bez publikacji, albo pytanie mailem **tylko** na adres z allowlisty (envelope From). Jednostka z hopu przekazującego, nie z envelope. Nie logować treści maila (wolno: model, intent, liczba szkiców, czas, kod błędu). Outbound do Vercel AI Gateway (Grok 4.1 Fast) z obrazami (JPEG stron PDF i zdjęcia, nie native PDF); timeout modelu 60 s / błąd = pytanie / Telegram, **nie** import 1:1 z tematu. Ponowny odczyt: `POST /api/admin/inbound/replay` (sesja administratora, ten sam `maxDuration`, 200 od razu). Mail zwrotny: From `wpisy@inbound.cncsolutions.dev`, DKIM/SPF domeny inbound.
...
| Webhook inbound `/api/inbound/email` | Podpis Svix; bez sesji; 401 przy złym podpisie; obcy From → 200 ignore |
