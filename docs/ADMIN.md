...
1. Temat i treść maila to punkt startowy. Grok ocenia **cały zestaw załączników** (także plakaty bez warstwy tekstowej) i redaguje jak człowiek: tytuł z tematu sprawy (nie „Plakaty”), krótki lead **jednej** sprawy, pismo przewodnie poza wpisem. Plakat / ulotka / zdjęcie idzie jako **podgląd**; uchwała — jako link. Gdy w jednej przesyłce jest kilka spraw — także spokrewnionych — powstaje do trzech szkiców. Ujęcia tej samej sprawy (inny podtytuł, strona, format) zostają w **jednym** wpisie; nie ma jednego wpisu na plik. Z DOCX **bez pieczątki i podpisu** grafiki idą do galerii (pierwsza = zajawka), a plik Word nie zostaje przy wpisie.
...
3. Grok dobiera kategorię z listy tej jednostki: **Aktualności** to domyślne miejsce na komunikat dla mieszkańców. Węższą (odpady, plan, smog, inwestycje, zarządzenie) tylko gdy materiał wyraźnie do niej należy. Ochrona ludności to stałe materiały kryzysowe — nie plakat weterynaryjny. I tak **sprawdź** tytuł, kategorię i podgląd załączników przed publikacją. Gdy Gateway nie odpowie, kategoria zostaje pusta, pliki lądują według heurystyki.
4. W `/admin/posts` filtr *Szkic* → *Edytuj*: sprawdź propozycję, popraw jeśli trzeba, potem *Wyślij do akceptacji* albo *Opublikuj szkic* (§5.2). Ponowny odczyt tego samego maila (np. po poprawce Groka): `POST /api/admin/inbound/replay` z `emailId` — kasuje poprzednie szkice z tej przesyłki i redaguje od nowa.
