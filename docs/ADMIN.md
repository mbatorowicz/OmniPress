...
1. Temat i treść maila to punkt startowy. Grok **ogląda** załączniki (obraz, skan PDF jako obraz strony, grafiki z Worda) i **redaguje jeden szkic** — tytuł jak news z nagłówka na plakacie, nie „Plakaty o…”. Daty i nazwę gminy przepisuje z plakatu. **Jeden mail = jeden wpis**; kolejne sprawy idą osobnymi mailami. Pismo przewodnie poza wpisem. Plakat / ulotka / zdjęcie idzie jako **podgląd**; uchwała — jako link. Z DOCX **bez pieczątki i podpisu** grafiki idą do galerii (pierwsza = zajawka), a plik Word nie zostaje przy wpisie.
...
3. Grok dobiera kategorię z listy tej jednostki: **Aktualności** to domyślne miejsce na komunikat dla mieszkańców. Węższą tylko gdy materiał wyraźnie do niej należy. Ochrona ludności to stałe materiały kryzysowe — nie plakat weterynaryjny. I tak **sprawdź** tytuł, kategorię i podgląd załączników przed publikacją.
4. Gdy mail prosi o **wymianę załącznika** w już opublikowanym wpisie albo na stronie stałej i Grok ma jeden pewny cel — w panelu otwiera się poprawka (na stronie zostaje stara wersja, aż opublikujesz). Gdy nie wiadomo który cel, albo skan jest nieczytelny — **nie powstaje nowy wpis**. Dostaniesz maila ze skrzynki `wpisy@` z pytaniem; odpowiedź w tym wątku idzie jeszcze raz przez Groka (te same załączniki + Twoja dopowiedź).
5. W `/admin/posts` filtr *Szkic* → *Edytuj*: sprawdź propozycję, popraw jeśli trzeba, potem *Wyślij do akceptacji* albo *Opublikuj szkic* (§5.2). Ponowny odczyt tego samego maila: `POST /api/admin/inbound/replay` z `emailId`.
6. Telegram (gdy bot działa) dostaje link do szkicu albo do podmiany, bez przycisku *Akceptuj*. Gdy Grok nie ogarnie przesyłki, wiadomość mówi o pytaniu na maila — nie szukaj wtedy nowego wpisu w kolejce.
...
- **Komponenty:** `/admin/units/[id]/components` — lista slotów (`home.*`, `header.brand`, `sidebar.weather`, `sidebar.cert_advisories`, `sidebar.recent_changes`, `sidebar.banner` itd.) w `omnipress-layout.json`. W **Nagłówek — logo i zdjęcia** ustawiasz herb oraz pasek zdjęć obok logo (jedno URL na linię; puste pole ukrywa pasek). Publikacja jak przy kategoriach. Wspólne pole kolejności (`order`) dla sidebaru. `sidebar.weather` i `sidebar.cert_advisories` pobierają dane **na żywo** z API na stronie Astro — bez syncu JSON do repo. Widget CERT zawsze dopełnia listę do limitu z panelu.
- **Strony statyczne:** `/admin/units/[id]/pages` — treści pod stałe URL. Lista przy otwarciu dociąga stan z GitHub; edytor i zapis szkicu czytają bazę (bez ponownego importu całego repo). Załączniki (PDF, DOCX, pliki) działają jak we wpisach: upload pod edytorem, edytowalna nazwa na stronie, PDF ma wybór link / podgląd. Publikacja nie nadpisze istniejącej treści pustym szkicem. *Utwórz strony z menu* dodaje tylko brakujące szkice (bez commita).
- **Wpisy:** przy wejściu na kolejkę / listę Omni dociąga opublikowane pliki z repo; szkice i kolejka nie są nadpisywane. Żeby panel znów był lustrem produkcji (szkice inbound znikają, podmienione wpisy wracają jako *Na stronie*, bez commita na GitHub): `npx tsx --tsconfig tsconfig.json scripts/reset-omni-to-live.ts`.
