...
4. W `/admin/posts` filtr *Szkic* → *Edytuj*: sprawdź propozycję, popraw jeśli trzeba, potem *Wyślij do akceptacji* albo *Opublikuj szkic* (§5.2). Ponowny odczyt tego samego maila (np. po poprawce Groka): `POST /api/admin/inbound/replay` z `emailId` — kasuje poprzedni szkic i redaguje od nowa.
